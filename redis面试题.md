...
|类型|简介|特性|使用场景|
|----|----|----|----|
|string| 二进制安全|string 类型的值最大能存储 512MB|常规key-value缓存应用。常规计数: 微博数, 粉丝数。|
|hash|Hash 是一个键值(key => value)对集合。Redis hash 是一个 string 类型的 field 和 value 的映射表，hash 特别适合用于存储对象。|hash的value是一个Map，这个Map的key是成员的属性名，value是属性值，这样对数据的修改和存取都可以直接通过其内部Map的Key(Redis里称内部Map的key为field), 也就是通过 key(用户ID) + field(属性标签) 就可以操作对应属性数据了，既不需要重复存储数据，也不会带来序列化和并发修改控制的问题 ，很好的解决了问题。|存储、读取、修改用户属性|
|list|双向链表|增删快，提供操作某段元素的api|1、最新消息排行等功能(比如朋友圈的时间线) 2、消息队列|
|set|哈希表实现元素不重复|1、添加、删除、查找的复杂度都是O(1)  2、为集合提供了求交集、并集、差集等操作|1、共同好友 2、利用唯一性,统计访问网站的所有独立ip 3、好友推荐时,根据tag求交集,大于某个阈值就可以推荐|
